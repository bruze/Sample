...
extension ViewController {
    ...
    func readAll() {
        let getRequest = RestRequest.init(url: client?.baseURL.absoluteString ?? "")
        getRequest.headerParameters = ["Accept": "application/json", "x-ibm-client-id": "853d3941-e376-4169-bc23-835a65b13a08"]
        getRequest.response { (data, httpResponse, error) in
            let decoder = JSONDecoder.init()
            let allToDoItems = try? decoder.decode([ToDo].self, from: data!)
